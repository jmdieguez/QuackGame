    ...
    try {
        ...
        switch (command.message.type) {
            case ClientActionType::MOVE_RIGHT:
                duck.move(Direction::RIGHT); break;

            case ClientActionType::MOVE_LEFT:
                duck.move(Direction::LEFT); break;

            case ClientActionType::STOP_MOVING_LEFT:
                duck.stop_moving(); break;
        ...
        ducks.emplace(command.player_id, Duck(command.player_id, 100, 200));
    }
}

void Game::step() {
    for (auto& [id, duck] : ducks) duck.step();
}
...
Snapshot Game::get_status() {
    ...
    for (auto& [id, duck] : ducks) duck_snapshots.push_back(duck.get_status());
