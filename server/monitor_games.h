...
public:
    MonitorGames();
    void create_game(const uint16_t &creator_id, Queue<Snapshot>&, Queue<ClientCommand>* game_queue);
    void add_player(const uint16_t&, const uint16_t &, Queue<Snapshot>&, Queue<ClientCommand>*);
