...
try
{
    ...
    // texture
    Texture sprites(renderer, Surface(ASSETS_PATH "/M484SpaceSoldier.png")
                                  .SetColorKey(true, 0));
    ...
    Texture gradient(renderer, SDL_PIXELFORMAT_BGR888, SDL_TEXTUREACCESS_STATIC, 1, 256);
    {
        ...
        std::generate(grad, grad + sizeof(grad), [&]()
                      { return 255 - n++ / 3; });
    ...
    // Load font, 12pt size
    Font font(ASSETS_PATH "/Vera.ttf", 12);
    ...
    while (1)
    {
        ...
        std::string text =
            "Position: " + std::to_string((int)position) + ", running: " + (is_running ? "true" : "false");
